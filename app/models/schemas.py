...
class Mitigation(BaseModel):
    category: str
    priority: str = Field(..., description="High, Medium, or Low implementation priority.")
    measure: str
    linked_risk: str = Field(..., description="Risk level that triggered this measure.")
...
class RiskDelta(BaseModel):
    ...
    changed: bool
    direction: str = Field(..., description="'up', 'down', 'same', or 'unknown'.")
...
class ComparisonResult(BaseModel):
    ...
    current_overall: RiskLevel
    overall_changed: bool
    deltas: list[RiskDelta]


class CompareRequest(BaseModel):
    baseline: AssessRequest
    current: AssessRequest


class CompareResultsBody(BaseModel):
    """Compare two already-computed assessments (e.g. pinned vs latest in the UI)."""
