...
def test_mitigations_generated_for_high_risk_demo():
    result = run_assessment(EXAMPLE_PROJECT)
    assert len(result.mitigations) >= 3
    assert any(m.category == "Biodiversity" for m in result.mitigations)
    assert all(m.measure and m.priority for m in result.mitigations)
    assert "## Recommended mitigations" in result.markdown


def test_sensitive_receptors_add_mitigations():
    project = EXAMPLE_PROJECT.model_copy(
        update={"sensitive_receptors": ["sacred grove", "migratory corridor"]}
    )
    result = run_assessment(project)
    cross = [m for m in result.mitigations if m.category == "Cross-cutting"]
    assert len(cross) == 2


def test_compare_detects_threshold_change():
    from app.core.compare import compare_results

    base = run_assessment(EXAMPLE_PROJECT)
    t = Thresholds(emissions_medium_tco2e=40_000)
    current = run_assessment(EXAMPLE_PROJECT, thresholds=t)
    cmp = compare_results(base, current)
    climate = next(d for d in cmp.deltas if d.category == "Climate")
    assert climate.changed
    assert climate.direction == "down"
    assert cmp.overall_changed or any(d.changed for d in cmp.deltas)
...
def test_geojson_export_endpoint():
    ...
    res = client.post(
        "/export/geojson",
        json={"project": EXAMPLE_PROJECT.model_dump()},
    )
    assert res.status_code == 200
    assert "geo+json" in res.headers["content-type"]
    body = res.json()
    assert body["type"] == "FeatureCollection"
    roles = {f["properties"]["role"] for f in body["features"]}
    assert "aoi" in roles
    assert "footprint" in roles
