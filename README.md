...
- **Preview AOI** before a full run (buffer distance and area on the map).
- **Toggle map layers** (protected areas, rivers, settlements, species, land cover, elevation).
- **Import GeoJSON** (paste or upload) for project geometry.
- **Save & load projects** in the browser (localStorage).
- **Sensitive receptors** field for proponent-flagged features (adds cross-cutting mitigations).
- **Activity presets** suggested when you change project type.
- **Metrics** and **Mitigations** tabs alongside the EIA report.
- **Assessment history** — re-run recent assessments from chips below the report.
- **Data warnings** banner when layers are missing.
- **Tune risk thresholds** in the collapsible editor and re-run to re-score.
- **Download** the report as **Markdown**, **DOCX**, or **PDF**, or **Print** a clean view.
- **Compare scenarios** — pin a baseline, change thresholds or project details, re-run, and view risk deltas.
- **Light / dark theme** with persisted preference.
- **GeoJSON export** of AOI + footprint (UI button or `POST /export/geojson`).
- **Share projects** via URL hash (`#p=…`) — copy link from the header.
- **Risk bar chart**, **copy markdown**, **JSON export**, **map basemap** switcher (OSM / Carto / satellite).
...
| GET | `/buffers` | AOI buffer distances (metres) by project type. |
...
| POST | `/export/geojson` | AOI + footprint as a GeoJSON FeatureCollection. |
| POST | `/compare` | Run two assessments and return risk deltas. |
| POST | `/compare/results` | Compare two cached `AssessmentResult` bodies. |
